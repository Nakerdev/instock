...
export default class Aes256EncryptionService implements EncryptionService {
  ...
  private readonly AES256_ALGORITHMS_NAME = 'aes256'
  ...
  encrypt (text: string): EncryptedResult {
    ...
    const cipher = createCipheriv(this.AES256_ALGORITHMS_NAME, Buffer.from(this.encryptionKey), iv)
  ...
  decrypt (encryptedResult: EncryptedResult): string {
    ...
    const decipher = createDecipheriv(this.AES256_ALGORITHMS_NAME, Buffer.from(this.encryptionKey), iv)
